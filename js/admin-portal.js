...
async function loadStatistics() {
    try {
        ...
        // Statistics errors are non-critical - silently fail
    }
}
...
async function releaseMidSemesterResults() {
    ...
    if (!confirm(`⚠️ RELEASE MID-SEMESTER RESULTS (STANDALONE + 15% TO FINAL)\n\nClass: ${className}\n\nThis will release mid-semester results as a STANDALONE result:\n\n📊 System will automatically:\n1. Accumulate ALL mid-semester scores from database:\n   - BFT 1 (Basic Field Training 1)\n   - Mid CS Exams (Mid Computer Science Exams)\n   - Mid Course Exercise\n   - ALL Quizzes/Tests accumulated during mid-semester (Quiz 1, Quiz 2, Quiz 3, etc.)\n\n2. Calculate overall mid-semester average percentage\n\n3. Calculate 15% contribution to final grade:\n   Formula: (Mid-semester average %) × 15% = Contribution to final\n   Example: If mid-semester average is 85%, contribution = 85% × 15% = 12.75%\n\n4. Release complete mid-semester results to student portal\n\n⚠️ IMPORTANT:\n   - Mid-semester is STANDALONE (students see it separately)\n   - Mid-semester ALSO contributes 15% to final semester grade\n   - System automatically calculates: (Mid average %) × 15%\n\nStudents will see Quiz 1, Quiz 2, Quiz 3, BFT 1, Mid CS Exams, Mid Course Exercise, and all scores with their mid-semester grades.\n\nAre you sure you want to proceed?`)) {
        return;
    }
...
async function releaseFinalSemesterResults() {
    ...
    if (!confirm(`⚠️ RELEASE FINAL SEMESTER RESULTS\n\nClass: ${className}\n\nThis will release final semester results:\n\n📊 System will automatically:\n1. Accumulate ALL final semester scores from database:\n   - ALL Quizzes accumulated throughout entire semester (Quiz 1, Quiz 2, Quiz 3, etc.)\n   - Final semester results: Opening Exams, BFT 2, Final Exams, Final Exercise\n\n2. Calculate final semester components with their normal percentages\n\n3. Add mid-semester 15% contribution:\n   Final Grade = (Mid-semester average % × 15%) + Final semester components\n   Example: If mid-semester average was 85% and final semester = 72%\n   → Final Grade = (85% × 15%) + 72% = 12.75% + 72% = 84.75%\n\n4. Calculate and grade everything automatically\n\n5. Release complete final semester results to student portal\n\n⚠️ IMPORTANT:\n   - Final semester includes ALL quizzes + Final Exams\n   - Mid-semester contributes 15% to final grade (automatically calculated)\n   - Mid-semester results remain STANDALONE (shown separately)\n\nStudents will see ALL quizzes, Final Exams, and complete final grade with mid-semester contribution.\n\nAre you sure you want to proceed?`)) {
        return;
    }
    ...
    try {
        ...
        // Release final semester results - EXCLUDES mid-semester (which is standalone)
        // Final semester includes: ALL quizzes throughout entire semester + Final semester exams only
        // Mid-semester (bft_1, mid_cs_exam, mid_course_exercise) is STANDALONE and NOT included
        let query = supabase
            .from('exams')
            .update({ 
                results_released: true,
            ...
            .in('exam_type', [
                // Final semester exams only (NOT mid-semester)
                'opening_exam', 'bft_2', 'final_exam', 'final_cse_exercise',
                // ALL Quizzes accumulated throughout entire semester (all quizzes, regardless of when taken)
        ...
        showSuccess(`✅ Final semester results released successfully for ${className}!\n\nThe system has:\n✓ Accumulated ALL final semester scores from database\n✓ Calculated final semester components with their normal percentages\n✓ Added mid-semester 15% contribution to final grade\n✓ Calculated complete final grade automatically\n✓ Released complete final semester results to student portal\n\n📊 Final Grade Calculation:\n   Formula: Final Grade = (Mid-semester average % × 15%) + Final semester components\n   Example: If mid-semester average = 85% and final semester = 72%\n   → Final Grade = (85% × 15%) + 72% = 12.75% + 72% = 84.75%\n\nStudents can now view:\n- ALL Quizzes throughout entire semester (Quiz 1, Quiz 2, Quiz 3, etc.)\n- Opening Exams scores\n- BFT 2 (Basic Field Training 2) scores\n- Final Exams scores (entered by lecturers)\n- Final Course Exercise scores\n- Complete final semester grade with mid-semester 15% contribution included\n\n⚠️ NOTE: Mid-semester results (BFT 1, Mid CS Exams, Mid Course Exercise) are STANDALONE but contribute 15% to final grade.\n\nAll results are now visible in the student portal.`, 'Success');
...
// Helper function to find or create admin user
async function findOrCreateAdminUser(supabase) {
    ...
    // No admin found - try to create one
    try {
        const systemAdminData = {
            username: 'system_admin_' + Date.now(),
            password: 'system_admin_temp_' + Date.now(),
            role: 'admin',
...
async function getOrCreateManualExam(classId, examType, subject) {
    ...
    // Always search for admin user in database first (don't trust localStorage ID)
    // This ensures we use a valid ID that exists in the database
    let adminUserId = null;
    
    // First, try to find any admin user in the database
    const { data: anyAdmin, error: adminError } = await supabase
        .from('users')
        .select('id')
        .eq('role', 'admin')
        ...
        try {
            ...
            if (!adminUserId) {
                ...
                if (!lecturerError && anyLecturer) {
                    ...
                    showError('No admin user found and unable to create one. Please create an admin user manually using the SQL script.', 'Database Error');
                    return null;
                }
            }
        } catch (createErr) {
    ...
    // Final verification: Double-check that adminUserId exists in database before using it
    ...
    const { data: newExam, error } = await supabase
        ...
        if (error.code === '23503') {
            ...
            const retryAdminId = await findOrCreateAdminUser(supabase);
            
            if (retryAdminId && retryAdminId !== adminUserId) {
                // Retry with the new admin user ID
                const { data: retryExam, error: retryError } = await supabase
                    .from('exams')
                    .insert([{
                        lecturer_id: retryAdminId,
                        title: `${examTypeDisplay} - ${subject} - ${formatClassName(classId)}`,
                        exam_type: examType,
                        subject: subject,
                        class_id: classId,
...
async function loadAnalytics() {
    try {
        ...
        // Analytics errors are non-critical - silently fail
    }
}
